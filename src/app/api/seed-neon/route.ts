// API Endpoint to Seed Neon Database - Production Safe
// POST /api/seed-neon
  ...
  try {
    ...
    if (!prisma) {
      return NextResponse.json({ 
        ...
        message: 'Please set DATABASE_URL environment variable in Vercel dashboard',
        instructions: {
          step1: 'Go to Vercel dashboard -> Project Settings -> Environment Variables',
          step2: 'Add DATABASE_URL with your Neon connection string',
          step3: 'Redeploy the application'
        }
      }, { status: 500 })
    }
    
    console.log('🌱 Starting Neon Database Seeding via API...')
    ...
    // Run seeding (simplified version)
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      message: 'Neon database status',
      tables: tableCounts,
      isEmpty: tableCounts.users === 0,
